package com.sulake.core.window.utils
{
   ...
   public class MouseEventProcessor implements IEventProcessor, IDisposable
   {
      ...
      protected static var var_283:Array;
      
      protected static var var_392:Point = new Point();
      ...
      protected var var_816:IWindowContextStateListener;
      ...
      public function MouseEventProcessor()
      {
         ...
         {
            var_326 = new Array();
            var_326[0] = MouseCursorType.const_289;
            var_326[1] = MouseCursorType.const_32;
            var_326[2] = MouseCursorType.const_289;
            var_326[3] = MouseCursorType.const_289;
            var_326[4] = MouseCursorType.const_289;
            var_326[5] = MouseCursorType.const_32;
            var_326[6] = MouseCursorType.const_289;
         }
         if(var_283 == null)
         {
            var_283 = new Array();
            var_283[0] = WindowState.const_106;
            var_283[1] = WindowState.const_84;
            var_283[2] = WindowState.const_99;
            var_283[3] = WindowState.const_70;
            var_283[4] = WindowState.const_54;
            var_283[5] = WindowState.const_100;
            var_283[6] = WindowState.const_82;
         }
      }
      
      public static function setMouseCursorByState(param1:uint, param2:uint) : void
      {
         var _loc3_:int = var_283.indexOf(param1);
         if(_loc3_ > -1)
         {
            var_326[_loc3_] = param2;
         }
      }
      ...
      protected static function convertMouseEventType(param1:MouseEvent, param2:IWindow, param3:IWindow) : WindowMouseEvent
      {
         ...
         switch(param1.type)
         {
            ...
            case MouseEvent.MOUSE_UP:
               ...
               _loc4_ = !!_loc6_ ? "null" : WindowMouseEvent.const_173;
               break;
      ...
      public function process(param1:EventProcessorState, param2:IEventQueue) : void
      {
         ...
         this.var_145 = state.desktop;
         this.var_72 = state.var_1252 as WindowController;
         this.var_171 = state.var_1251 as WindowController;
         this.var_164 = state.renderer;
         this.var_816 = state.var_1249;
         eventQueue.begin();
         this.var_657.x = -1;
         this.var_657.y = -1;
         ...
         while(true)
         {
            ...
            if(event.stageX != this.var_657.x || event.stageY != this.var_657.y)
            {
               this.var_657.x = event.stageX;
               this.var_657.y = event.stageY;
               array = new Array();
               this.var_145.groupParameterFilteredChildrenUnderPoint(this.var_657,array,WindowParam.const_31);
            }
            ...
            if(index == 0)
            {
               ...
               {
                  if(this.var_72 != this.var_145 && !this.var_72.disposed)
                  {
                     this.var_72.getGlobalPosition(var_392);
                     ...
                     this.var_72 = WindowController(this.var_145);
                  }
               }
               else if(event.type == MouseEvent.MOUSE_DOWN)
               {
                  window = this.var_145.getActiveWindow();
                  if(window)
                  {
                     window.deactivate();
                  }
               ...
               if(child != null && child.visible)
               {
                  if(event.type == MouseEvent.MOUSE_MOVE)
                  {
                     if(child != this.var_72)
                     {
                        ...
                        if(!child.disposed)
                        {
                           child.getGlobalPosition(var_392);
                  ...
                  else if(event.type == MouseEvent.MOUSE_UP || event.type == MouseEvent.CLICK)
                  {
                     if(this.var_72 && !this.var_72.disposed)
                     {
                        ...
                        {
                           this.var_816.mouseEventReceived(event.type,this.var_72);
                        }
                     }
                  }
                  temp = child.parent;
         ...
         state.desktop = this.var_145;
         state.var_1252 = this.var_72;
         state.var_1251 = this.var_171;
         state.renderer = this.var_164;
         state.var_1249 = this.var_816;
      }
      ...
      private function passMouseEvent(param1:WindowController, param2:MouseEvent, param3:Boolean = false) : WindowController
      {
         ...
         if(param1.testParamFlag(WindowParam.const_501))
         {
