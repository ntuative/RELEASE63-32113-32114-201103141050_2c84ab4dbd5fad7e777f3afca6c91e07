package com.sulake.core.utils
{
   ...
   public class LibraryLoaderQueue extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1663:int = 4;
      ...
      {
         return this.var_406.length + this.var_278.length;
      }
      ...
      private function next() : void
      {
         ...
         if(!disposed)
         {
            while(this.var_278.length < const_1663 && this.var_406.length > 0)
            {
               _loc1_ = this.var_406.shift();
      ...
      private function removeLoader(param1:LibraryLoader) : void
      {
         ...
         try
         {
            index = this.var_406.indexOf(loader);
            if(index > -1)
            {
               this.var_406.splice(index,1);
            }
            index = this.var_278.indexOf(loader);
            if(index > -1)
            {
               this.var_278.splice(index,1);
            }
         }
         catch(e:Error)
         {
      ...
      private function isUrlInQueue(param1:String, param2:Boolean = true) : Boolean
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_406)
            {
               if(param2)
               {
      ...
      public function findLibraryLoaderByURL(param1:String, param2:Boolean = true) : LibraryLoader
      {
         ...
         if(!disposed)
         {
            ...
            for each(_loc3_ in this.var_278)
            {
               if(param2)
               {
                  if(_loc3_.url.indexOf(param1) == 0)
