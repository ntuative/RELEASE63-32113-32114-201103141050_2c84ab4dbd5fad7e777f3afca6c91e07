package com.sulake.core.utils
{
   ...
   public class LibraryLoader extends EventDispatcher implements IDisposable
   {
      ...
      protected static const const_1678:uint = 1;
      
      protected static const const_1681:uint = 2;
      
      protected static const const_1679:uint = 4;
      
      protected static const const_1680:uint = 3;
      ...
      public static const const_1104:Boolean = false;
      
      public static const const_1663:int = 2;
      
      private static var var_828:Array = new Array();
      
      private static var var_666:Array = new Array();
      ...
      protected var var_388:int = 0;
      
      protected var var_233:URLRequest;
      
      protected var var_751:XML;
      
      protected var var_611:Class;
      
      protected var var_447:uint = 0;
      
      protected var _name:String;
      
      protected var var_1011:Boolean = false;
      
      protected var _debug:Boolean = false;
      
      protected var var_1012:Boolean = false;
      
      protected var var_2784:String = "";
      
      protected var var_2782:String = "";
      
      protected var var_2013:int;
      
      protected var var_1254:int;
      
      protected var var_1253:int;
      
      protected var var_2875:uint = 0;
      ...
      protected static function queue(param1:LibraryLoader) : void
      {
         ...
         {
            param1.var_42.load(param1.var_233,param1._context);
         }
      }
      ...
      protected static function throttle() : void
      {
         ...
         if(const_1104)
         {
            ...
            while(_loc2_ > -1)
            {
               _loc1_ = var_666[_loc2_] as LibraryLoader;
               if(_loc1_ && (_loc1_.ready || _loc1_.disposed))
               {
                  var_666.splice(_loc2_,1);
               }
               _loc2_--;
            }
            while(var_666.length < const_1663 && false)
            {
               _loc1_ = var_828.shift() as LibraryLoader;
               if(!_loc1_.ready && !_loc1_.disposed)
               {
                  var_666.push(_loc1_);
                  _loc1_.var_42.load(_loc1_.var_233,_loc1_._context);
               }
            }
         }
      }
      ...
      {
         return !!this.var_233 ? this.var_233.url : null;
      }
      ...
      {
         return !!this.var_1011 ? uint(this.var_1254 - this.var_2013) : uint(getTimer() - this.var_2013);
      }
      ...
      override public function dispose() : void
      {
         if(!disposed)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DISPOSE,this.var_388,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
            try
            {
               this.var_42.close();
            }
      ...
      public function load(param1:URLRequest, param2:int = 5) : void
      {
         ...
         this._name = parseNameFromUrl(this.var_233.url);
      ...
      {
         if(this.var_1012 && !_disposed)
         {
            this.var_1012 = false;
            if(!this.var_1011 && this.var_233)
            {
               this.var_1254 = -1;
               this.var_2013 = getTimer();
               queue(this);
            }
         }
      }
      ...
      {
         if(!this.var_1011 && !_disposed && this.var_1253 > 0)
         {
            try
            {
               this.var_42.close();
            ...
            this.addRequestCounterToUrlRequest(this.var_233,const_1557 - this.var_1253);
            if(this.var_2783)
            {
               this.var_233.requestHeaders.push(new URLRequestHeader("pragma","no-cache"));
               this.var_233.requestHeaders.push(new URLRequestHeader("Cache-Control","no-cache"));
               this.var_2783 = false;
            }
            --this.var_1253;
            this.var_447 = 0;
            this.var_42.load(this.var_233,this._context);
            return true;
         }
         return false;
      }
      ...
      protected function loadEventHandler(param1:Event) : void
      {
         ...
         switch(param1.type)
         {
            ...
            case HTTPStatusEvent.HTTP_STATUS:
               this.var_388 = HTTPStatusEvent(param1).status;
               this.debug("Load event STATUS " + this.var_388 + " for file \"" + this.url + "\"");
               break;
            case Event.UNLOAD:
               this.debug("Load event UNLOAD for file \"" + this.url + "\"");
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_UNLOAD,this.var_388,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case ProgressEvent.PROGRESS:
               this.debug("Load event PROGRESS for file \"" + this.url + "\"  bytes: " + this.bytesLoaded + "/" + this.bytesTotal);
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_PROGRESS,this.var_388,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
               break;
            case IOErrorEvent.IO_ERROR:
               this.debug("Load event IO ERROR for file \"" + this.url + "\"");
               if(!this.handleHttpStatus(this.var_388))
               {
         ...
         if(this.var_447 == LibraryLoader.const_1680)
         {
         ...
         if(this.var_447 == LibraryLoader.name_3)
         {
            if(this.prepareLibrary())
            {
               ...
               this.removeEventListeners();
               throttle();
               dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_COMPLETE,this.var_388,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
      ...
      protected function prepareLibrary() : Boolean
      {
         ...
         this.var_611 = this.getDefinition(this._name) as Class;
         if(this.var_611 == null)
         {
            if(!this.retry())
            {
               this.failure("Failed to find resource class \"" + this._name + "\" from library " + this.var_233.url + "!");
            }
            return false;
         }
         try
         {
            xmlClass = this.var_611.manifest as Class;
            if(xmlClass == null)
            {
               return false;
            }
         ...
         try
         {
            this.var_751 = new XML(bytes.readUTFBytes(bytes.length));
         }
         catch(e:Error)
         {
            if(!retry())
      ...
      protected function debug(param1:String) : void
      {
         ...
         if(this._debug)
         {
            dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_DEBUG,this.var_388,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         }
      }
      ...
      protected function failure(param1:String) : void
      {
         ...
         throttle();
         dispatchEvent(new LibraryLoaderEvent(LibraryLoaderEvent.LIBRARY_LOADER_EVENT_ERROR,this.var_388,this.bytesTotal,this.bytesLoaded,this.elapsedTime));
         dispatchEvent(new Event(LIBRARY_LOADER_FINALIZE));
      }
