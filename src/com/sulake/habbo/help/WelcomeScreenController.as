package com.sulake.habbo.help
{
   ...
   public class WelcomeScreenController implements IUpdateReceiver
   {
      ...
      public function WelcomeScreenController(param1:HabboHelp, param2:IHabboWindowManager, param3:IHabboConfigurationManager)
      {
         ...
         this._config = param3;
         this.var_138.toolbar.events.addEventListener(HabboToolbarEvent.const_37,this.onToolbarClicked);
         this.var_138.toolbar.events.addEventListener(HabboToolbarEvent.const_1126,this.onToolbarResized);
         var _loc4_:Boolean = Boolean(parseInt(this._config.getKey("new.identity","0")));
         this.var_1912 = Boolean(parseInt(this._config.getKey("welcome.screen.enabled","0"))) && _loc4_;
         this.var_2694 = parseInt(this._config.getKey("welcome.screen.delay","5000"));
         this.var_342 = new Point(72,10);
         this.var_199 = new Timer(this.var_2694,1);
      ...
      public function dispose() : void
      {
         ...
         if(this.var_138)
         {
            this.var_138.toolbar.events.removeEventListener(HabboToolbarEvent.const_37,this.onToolbarClicked);
            this.var_138 = null;
         }
      ...
      public function setCategory(param1:String, param2:String = "", param3:String = "") : void
      {
         ...
         {
            this.var_1913 = "welcome.screen.title";
         }
         if(param3.length > 0)
         {
         ...
         {
            this.var_1911 = "welcome.screen.message";
         }
      }
      
      private function onTimerComplete(param1:TimerEvent) : void
      {
         var _loc2_:XmlAsset = this.var_138.assets.getAssetByName("welcome_screen_xml") as XmlAsset;
         ...
         var _loc4_:BitmapDataAsset = this.var_138.assets.getAssetByName("welcome_screen_arrow") as BitmapDataAsset;
         ...
         _loc5_.content.setParamFlag(WindowParam.const_295,false);
         _loc5_.height -= 20;
         this._window.findChildByName("close").addEventListener(WindowMouseEvent.WINDOW_EVENT_MOUSE_CLICK,this.onMouseClick);
         this.var_138.registerUpdateReceiver(this,10);
         this.var_1912 = false;
         this.var_342.y = this.var_138.toolbar.getIconLocation(this.var_1479) - this._window.height / 2;
         this._window.y = this.var_342.y;
         this._window.findChildByName("title").caption = "${" + this.var_1913 + "}";
         this._window.findChildByName("text").caption = "${" + this.var_1911 + "}";
         this.highlightToolbarIcon(true);
      }
      
      private function highlightToolbarIcon(param1:Boolean) : void
      {
         var _loc2_:HabboToolbarSetIconEvent = new HabboToolbarSetIconEvent(HabboToolbarSetIconEvent.const_91,this.var_1479);
         _loc2_.iconState = !!param1 ? "highlight_blue" : "0";
         this.var_138.toolbar.events.dispatchEvent(_loc2_);
      }
      ...
      private function onToolbarResized(param1:HabboToolbarEvent) : void
      {
         ...
         if(this.var_138)
         {
            this.var_138.removeUpdateReceiver(this);
            this.var_138.registerUpdateReceiver(this,10);
         }
         this.var_342.y = this.var_138.toolbar.getIconLocation(this.var_1479) - this._window.height / 2;
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         var _loc2_:Number = Point.distance(this._window.rectangle.topLeft,this.var_342);
         if(_loc2_ > 5)
         {
            _loc3_ = Point.interpolate(this._window.rectangle.topLeft,this.var_342,0.5);
            this._window.x = _loc3_.x;
            this._window.y = _loc3_.y;
         }
         else
         {
            this._window.x = this.var_342.x;
            this._window.y = this.var_342.y;
            this.var_138.removeUpdateReceiver(this);
         }
      }
