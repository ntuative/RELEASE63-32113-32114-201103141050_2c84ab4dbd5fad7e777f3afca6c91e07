package com.sulake.habbo.window
{
   ...
   public class HabboWindowManagerComponent extends Component implements IHabboWindowManager, ICoreWindowManager, IWindowFactory, IUpdateReceiver, IWindowContextStateListener
   {
      ...
      private static const const_411:uint = 3;
      
      private static const const_1259:uint = 1;
      ...
      private var var_1297:IEventDispatcher;
      
      private var var_1050:Boolean = false;
      
      private var var_1568:ProfilerOutput;
      
      private var var_1298:ProfilerAgentTask;
      ...
      private function initialize(param1:IID, param2:IUnknown) : void
      {
         ...
         this._windowContextArray = new Array(const_411);
         ...
         var _loc6_:int = 0;
         while(_loc6_ < const_411)
         {
            this._windowContextArray[_loc6_] = new WindowContext("layer_" + _loc6_,this.var_851,this,this._localization,context.displayObjectContainer,_loc5_,this);
            _loc6_++;
         }
         this.var_267 = this._windowContextArray[const_1259];
         unlock();
      ...
      private function receiveProfilerInterface(param1:IID, param2:IUnknown) : void
      {
         ...
         if(_loc3_ != null)
         {
            if(!this.var_1568)
            {
               this.var_1568 = new ProfilerOutput(context,this,this.var_851);
            }
            this.var_1568.profiler = _loc3_;
            this.var_1298 = new ProfilerAgentTask("Update","Event processing");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1298);
            this.var_1299 = new ProfilerAgentTask("Redraw","Window rasterizer");
            _loc3_.getProfilerAgentForReceiver(this).addSubTask(this.var_1299);
            this.var_1050 = true;
         }
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         if(WindowContext.var_387.length > 0)
         {
            this.var_1297.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_INPUT));
            _loc2_ = const_411 - 1;
            while(_loc2_ >= 0)
            {
         ...
         this.var_1297.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_RENDER));
         _loc2_ = 0;
         while(_loc2_ < const_411)
         {
         ...
         if(WindowContext.var_387.length > 0)
         {
            WindowContext.var_387.flush();
         }
         MouseCursorControl.change();
         this.var_1297.dispatchEvent(new Event(HabboWindowTrackingEvent.HABBO_WINDOW_TRACKING_EVENT_SLEEP));
      }
      ...
      public function mouseEventReceived(param1:String, param2:IWindow) : void
      {
         if(param2 != null)
         {
            ...
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1649,new Date().getTime().toString());
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1540,param2.name + ": " + param2.toString());
            }
            else if(param1 == MouseEvent.MOUSE_UP)
            {
               ErrorReportStorage.setParameter(HabboErrorVariableEnum.const_1524,new Date().getTime().toString());
