package com.sulake.habbo.toolbar
{
   ...
   public class ToolbarIconAnimator
   {
      ...
      public function ToolbarIconAnimator(param1:StateItem, param2:IAssetLibrary, param3:IBitmapWrapperWindow, param4:Number, param5:Number, param6:BitmapData = null)
      {
         ...
         this.var_1352 = param1.loop;
         if(param1.background != null)
         {
            this.var_726 = this.getBitmapDataFromAsset(param1.background,param2);
         }
         ...
         if(_loc9_.length() > 0)
         {
            ...
            while(_loc10_ < _loc9_.length())
            {
               ...
               if(_loc8_ != null)
               {
                  this.var_897.push(_loc11_);
                  this.var_896.push(_loc8_);
               }
               _loc10_++;
            }
         }
         else
         {
            ...
            if(_loc8_ != null)
            {
               this.var_897.push(new XML());
               this.var_896.push(_loc8_);
            }
         }
         if(param6 != null)
         {
            this.var_896[this._frame] = param6.clone();
         }
      ...
      {
         if(this.var_1352 == true || this.var_898 == 0)
         {
            if(this.var_897.length > 0)
            {
               ++this._frame;
               if(this._frame >= this.var_897.length)
               {
                  this._frame = 0;
                  ++this.var_898;
               }
      ...
      {
         return this.var_897.length;
      }
      
      public function hasNextState() : Boolean
      {
         return this.var_1352 == false && this.var_898 == 1 && this.var_1627 != this._state;
      }
      ...
      {
         var _loc2_:* = null;
         if(this._frame <= this.var_896.length)
         {
            _loc2_ = this.var_896[this._frame];
      ...
      private function setAsset(param1:IBitmapWrapperWindow, param2:BitmapData) : void
      {
         ...
         {
            _loc3_ = new BitmapData(Math.max(param2.width,this.var_726.width),Math.max(param2.height,this.var_726.height),true,0);
            _loc4_ = _loc3_.height - this.var_726.rect.height;
            _loc3_.copyPixels(this.var_726,this.var_726.rect,new Point(0,_loc4_),null,null,true);
