package com.sulake.habbo.room.object.visualization.furniture
{
   ...
   public class FurnitureParticleSystem
   {
      ...
      private var var_1378:ColorTransform;
      
      private var var_1745:Matrix;
      ...
      public function FurnitureParticleSystem(param1:AnimatedFurnitureVisualization)
      {
         ...
         this.var_1378 = new ColorTransform();
         this.var_1745 = new Matrix();
      ...
      public function setAnimation(param1:int) : void
      {
         ...
         this.var_96 = this.var_1139[param1];
         this.var_918 = false;
         this.updateCanvas();
      }
      ...
      private function updateCanvas() : void
      {
         ...
         if(this.var_1744 >= 0)
         {
            this.var_262 = this._visualization.getSprite(this.var_1744);
            if(this.var_262 && this.var_262.asset)
            {
               ...
               if(this.var_86 == null)
               {
                  this.var_86 = this.var_262.asset.clone();
                  if(this.var_743.alphaMultiplier != 1)
                  {
                     this.var_605 = new BitmapData(this.var_86.width,this.var_86.height,true,4278190080);
                  }
               }
               this.var_2290 = -this.var_262.offsetX;
               this.var_2289 = -this.var_262.offsetY;
               this.var_262.asset = this.var_86;
            }
            if(this.var_86)
            {
      ...
      public function getSpriteYOffset(param1:int, param2:int, param3:int) : int
      {
         ...
         {
            return this.var_96.y * this.var_1138;
         }
         return 0;
      }
      ...
      {
         if(!this.var_96 || !this.var_262)
         {
            return;
         }
         if(this.var_86 && this.var_262.asset != this.var_86)
         {
            this.var_262.asset = this.var_86;
         }
         if(this.var_918)
      ...
      public function updateAnimation() : void
      {
         ...
         var _loc2_:* = 0;
         var _loc8_:int = 0;
         if(!this.var_918 && this.var_96.hasIgnited)
         {
            this.var_918 = true;
         }
         _loc8_ = this.var_1213 * this.var_1138;
         this.var_96.update();
         if(this.var_918)
         {
            if(this.var_96.roomObjectSpriteId >= 0)
            ...
            {
               this.var_86.draw(this.var_605,this.var_1745,this.var_743,BlendMode.NORMAL,null,false);
            }
            for each(_loc11_ in this.var_96.particles)
            {
               _loc2_ = Number(_loc11_.y);
               _loc3_ = this.var_2290 + (_loc11_.x - _loc11_.z) * 10 / 10 * this.var_1138;
               _loc4_ = this.var_2289 - _loc8_ + (_loc2_ + (_loc11_.x + _loc11_.z) / 2) * 10 / 10 * this.var_1138;
               ...
               if(_loc6_)
               {
                  ...
                  if(_loc11_.fade && _loc11_.alphaMultiplier < 1)
                  {
                     ...
                     this.var_1378.alphaMultiplier = _loc11_.alphaMultiplier;
                     this.var_86.draw(_loc7_,this._translationMatrix,this.var_1378,BlendMode.NORMAL,null,false);
                  }
                  else
                  {
      ...
      public function parseData(param1:XML) : void
      {
         ...
         var _loc23_:* = null;
         var _loc24_:* = null;
         this.var_2444 = parseInt(param1.@size);
         this.var_1744 = !!param1.hasOwnProperty("@canvas_id") ? int(parseInt(param1.@canvas_id)) : -1;
         this.var_1213 = !!param1.hasOwnProperty("@offset_y") ? int(parseInt(param1.@offset_y)) : 10;
         this.var_1138 = this.var_2444 / 64;
         this.var_699 = !!param1.hasOwnProperty("@blend") ? Number(Number(param1.@blend)) : Number(1);
         this.var_699 = Math.min(this.var_699,1);
         this.var_743.alphaMultiplier = this.var_699;
         for each(_loc3_ in param1.emitter)
         {
            ...
            for each(_loc23_ in _loc3_.particles.particle)
            {
               ...
               _loc20_ = _loc23_.hasOwnProperty("@fade") && _loc23_.@fade == "HabboAchievementShareIdMessageParser" ? true : false;
               _loc21_ = [];
