package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private const const_1970:int = 1000;
      ...
      private var var_839:IWindow;
      
      public function EffectView()
      {
         super();
         this.var_199 = new Timer(this.const_1970);
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         if(this.var_175.isActive)
         {
            _loc2_ = this.var_175.secondsLeft / Number(this.var_175.duration);
            this.var_840.width = _loc2_ * this.var_293;
         }
         else
         {
            this.var_840.width = 0;
            this.var_199.stop();
         }
         this.setTimeLeft();
      }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_671 = this._window.findChildByName("time_left") as ITextWindow;
            if(this.var_671 == null)
            {
               return;
            }
         ...
         if(!this.var_175.isActive)
         {
            this.var_671.caption = "${widgets.memenu.effects.activate}";
            return;
         }
         this.var_671.caption = "${widgets.memenu.effects.active.timeleft}";
         var _loc1_:String = this.var_671.text;
         var _loc2_:int = this.var_175.secondsLeft;
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_175.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_175.isActive)
         {
         ...
         else
         {
            ...
            this.var_839 = this._window.findChildByName("effect_hilite");
            this.var_839.visible = false;
         }
         this.setTimeLeft();
         this.var_840 = this._window.findChildByName("loader_bar");
         if(this.var_840 != null)
         {
            this.var_293 = this.var_840.width;
            this.var_199.start();
            this.onUpdate();
         }
         if(this.var_175.icon)
         {
            this.setElementBitmap("effect_icon",this.var_175.icon);
         }
      }
