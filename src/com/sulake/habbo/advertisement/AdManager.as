package com.sulake.habbo.advertisement
{
   ...
   public class AdManager extends Component implements IAdManager
   {
      ...
      private var var_1204:BitmapData = null;
      ...
      {
         this.var_319 = param2 as IHabboCommunicationManager;
         if(this.var_319 == null)
         {
            return;
         }
         this._connection = this.var_319.getHabboMainConnection(this.onConnectionReady);
      ...
      private function onConfigurationReady(param1:IID = null, param2:IUnknown = null) : void
      {
         ...
         this.var_2631 = this._config.getKey("interstitial.enabled","false") == "HabboAchievementShareIdMessageParser";
         this.var_2630 = int(this._config.getKey("interstitial.interval","30000"));
         this.var_2633 = int(this._config.getKey("interstitial.show.time","5000"));
         this.var_1874 = int(this._config.getKey("interstitial.max.displays.flash","-1"));
      ...
      public function showInterstitial() : Boolean
      {
         ...
         if(this.var_222 != null && this.var_222.running)
         {
            this.var_222.reset();
            return false;
         }
         if(this.var_1202 == 0)
         {
            this.var_1202 = getTimer();
            return false;
         }
         if(this.var_1874 > 0 && this.var_1873 >= this.var_1874)
         {
            return false;
         }
         var _loc1_:int = getTimer() - this.var_1202;
         if(_loc1_ > this.var_2630)
         {
            ++this.var_1873;
      ...
      private function onInterstitial(param1:InterstitialMessageEvent) : void
      {
         ...
         this.var_1456 = _loc2_.imageUrl;
         this.var_2632 = _loc2_.clickUrl;
         this.var_1202 = getTimer();
         if(this.var_1456 != "")
         {
            _loc3_ = this.var_1456;
            ...
            {
               this.var_222 = new Timer(this.var_2633,1);
               this.var_222.addEventListener(TimerEvent.TIMER_COMPLETE,this.interstitialTimerComplete);
            }
            if(this.var_222.running)
            {
               this.var_222.reset();
            }
            this.var_222.start();
            _loc4_ = new URLRequest(this.var_1456);
      ...
      private function startInterstitial(param1:Bitmap) : void
      {
         ...
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_549,0,0,param1.bitmapData,this.var_2632));
            this.var_1202 = getTimer();
         }
         else
         {
      ...
      private function stopInterstitial() : void
      {
         ...
         if(events != null)
         {
            events.dispatchEvent(new AdEvent(AdEvent.const_560));
         }
      }
      ...
      private function onRoomAd(param1:RoomAdMessageEvent) : void
      {
         ...
         this.var_1455 = _loc2_.imageUrl;
         this.var_1875 = _loc2_.clickUrl;
         if(this.var_1455 != "" && this.var_1875 != "")
         {
            _loc3_ = this.var_1455;
            if(assets.hasAsset(_loc3_))
            {
               assets.removeAsset(assets.getAssetByName(_loc3_));
            }
            _loc4_ = new URLRequest(this.var_1455);
            _loc5_ = assets.loadAssetFromFile(_loc3_,_loc4_,"image/jpeg");
      ...
      private function roomAdImageReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(this.isValidAdImage(_loc3_))
         {
            ...
            events.dispatchEvent(new AdEvent(AdEvent.const_542,0,0,_loc4_,this.var_1875,this.var_1204,this.var_1203));
         }
      }
      ...
      private function adWarningLeftReady(param1:AssetLoaderEvent) : void
      {
         ...
         if(_loc3_ != null)
         {
            this.var_1204 = this.emulateBackgroundTransparency(_loc3_.bitmapData);
         }
      }
