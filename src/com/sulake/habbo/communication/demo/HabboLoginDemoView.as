package com.sulake.habbo.communication.demo
{
   ...
   public class HabboLoginDemoView extends EventDispatcher
   {
      ...
      public static const const_512:String = "InitConnection";
       
      
      private var var_165:HabboCommunicationDemo;
      ...
      private const const_1710:String = "fuselogin";
      ...
      {
         this.var_165.windowManager.removeWindow("habbo_login_window");
      }
      ...
      {
         var _loc1_:XmlAsset = (this.var_165 as Component).assets.getAssetByName("login_window") as XmlAsset;
         if(_loc1_ == null)
         {
            Logger.log("Could  not find embedded Window XML for Login Window, login_window");
            return;
         }
         this._window = this.var_165.windowManager.createWindow("habbo_login_window","Login",HabboWindowType.const_153,HabboWindowStyle.const_43,0 | 0,new Rectangle(10,10,320,240),null) as IFrameWindow;
         var _loc2_:XML = _loc1_.content as XML;
         var _loc3_:IWindowContainer = this.var_165.windowManager.buildFromXML(_loc2_) as IWindowContainer;
         ...
         var _loc7_:SharedObject = SharedObject.getLocal(this.const_1710,"/");
         if(_loc5_ != null)
         {
         ...
         if(_loc8_)
         {
            if(this.var_165.habboConfiguration)
            {
               if(this.var_165.habboConfiguration.getKey("use.sso","false") == "HabboAchievementShareIdMessageParser")
               {
                  _loc8_.select();
               }
            }
      ...
      public function populateUserList(param1:Map) : void
      {
         ...
         var _loc3_:XmlAsset = this.var_165.assets.getAssetByName("login_user_list_item") as XmlAsset;
         var _loc4_:IWindow = this.var_165.windowManager.buildFromXML(_loc3_.content as XML);
      ...
      private function windowEventProcessor(param1:WindowEvent = null, param2:IWindow = null) : void
      {
         ...
         if(_loc3_ != null)
         {
            ...
            _loc6_ = SharedObject.getLocal(this.const_1710,"/");
         ...
         {
            dispatchEvent(new Event(const_512));
         }
      }
      ...
      private function listEventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         this.var_165.sendTryLogin(this.name,this.password,param2.id);
      }
      ...
      private function initSSOTicket(param1:Boolean) : void
      {
         ...
         {
            this.var_135.removeEventListener(SSOTicket.const_408,this.onParsedTicketSuccess);
            this.var_135.removeEventListener(SSOTicket.const_318,this.onParsedTicketFailure);
            this.var_135 = null;
         }
         var _loc2_:String = "";
         if(this.var_165.habboConfiguration != null)
         {
            _loc2_ = this.var_165.habboConfiguration.getKey("url.prefix",_loc2_);
            _loc2_ = _loc2_.replace("http://","");
            _loc2_ = _loc2_.replace("https://","");
         }
         var _loc3_:IHabboWebLogin = this.var_165.habboCommunication.habboWebLogin(this.name,this.password);
         this.var_135 = new SSOTicket(this.var_165.assets,_loc3_,_loc2_,param1);
         this.var_135.addEventListener(SSOTicket.const_408,this.onParsedTicketSuccess);
         this.var_135.addEventListener(SSOTicket.const_318,this.onParsedTicketFailure);
      }
      ...
      private function onParsedTicketSuccess(param1:Event) : void
      {
         ...
         {
            this.var_165.habboConfiguration.setKey("facebook.user","1");
         }
         this.var_165.shockwaveClientUrl = this.var_135.shockwaveClientUrl;
         this.var_165.ssoTicket = this.var_135.ticket;
         this.var_135.removeEventListener(SSOTicket.const_408,this.onParsedTicketSuccess);
         this.var_135.removeEventListener(SSOTicket.const_318,this.onParsedTicketFailure);
         this.var_135 = null;
         dispatchEvent(new Event(const_512));
